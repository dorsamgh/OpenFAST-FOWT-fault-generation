...
!bjj: there is a serious problem with the debugger here when DET = 0
   ...
   !--------------------------------------------------------------
   SUBROUTINE Line_GetStateDeriv(Line, Xd, m, p, t)  !, FairFtot, FairMtot, AnchFtot, AnchMtot)
      ...
      DO I = 1, N
         ...
         do J = 1, 3
			...
			IF (t .GT. 400.0 .AND. Line%IdNum == 1 .AND. I == N) THEN
				Line%T(:,I) = Line%T(:,I) * (1.0 + 0.01 * SIN(0.1 * (t - 400.0)))
				Line%Td(:,I) = Line%Td(:,I) * (1.0 + 0.01 * SIN(0.1 * (t - 400.0)))
			END IF
